...
work/
  ...
  _template/
    record.md                   # 日常记录的模板（用户可改，CLAUDE.md 只链接）
    table.md                    # 表格周报版严格遵守的模板
    habbit.md                   # 学习到的用户写作 / 发言习惯（持续更新）
  report/                       # 生成的周报产出（按周归档）
  TODO.md                       # 活维护的待办列表，每次记录都要同步更新
```
...
- 内容**严格按 [`work/_template/record.md`](work/_template/record.md) 模板填写**，不要在 CLAUDE.md 里复制模板内容——用户可能随时改模板，链接引用即可。
- 一次会话内多次更新同一文件即可，不要拆成多个文件。
- 写完当日记录后，**必须同步更新 [`work/TODO.md`](work/TODO.md)**：已完成项划掉或移到「已完成」分区，新增的待办补进「待办」分区。
